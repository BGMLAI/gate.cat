"""cacheback.veto — action-veto: zatrzymaj agenta ZANIM zrobi nieodwracalną akcję.
...
GatedLoop (cacheback.agent) pilnuje RZEKI: model się waha (rozrzut) → pauza pętli.
...
Trzy mury (fail-closed — błąd któregokolwiek = VETO, nie przepuszczenie):
  ...
  2. KORYTO — gdy akcja ma sprawdzalny atom, interpreter weryfikuje NIEZALEŻNIE
              od modelu (cacheback.koryto, recall 1.0, 0% false-pass w proxy).
...
Użycie:
    from cacheback.veto import before_action, ActionPolicy, ActionVetoed
