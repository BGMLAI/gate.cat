...
def koryto_exec_python(stmts: Sequence[str], timeout: float = 5.0) -> Optional[str]:
    ...
    if not stmts:
        return None
    try:
        from cacheback.koryto_sandbox import run_context_guard
...
def koryto_exec_node(code: str, timeout: float = 10.0) -> Optional[str]:
    ...
    constructor.constructor, fromCharCode obchodzą każdą deny-listę). Dlatego:
      - DOMYŚLNIE WYŁĄCZONY. Wymaga jawnego `CACHEBACK_KORYTO_EXEC_NODE_UNSAFE=1`
    ...
    """
    if os.environ.get("CACHEBACK_KORYTO_EXEC_NODE_UNSAFE", "0").strip() not in ("1", "true", "True"):
        return None
    import shutil
