...
Usage:
    ...
    # Async
    from cacheback import AsyncCachedOpenAI
...
class _CachedResponse:
    ...
    def __init__(
        ...
        self.cacheback_hit = cache_hit
        self.cacheback_synthesized = synthesized
    ...
    def __getattr__(self, name):
        if self._original and name not in (
            "_original", "cacheback_hit", "_cache_hit", "cacheback_synthesized",
        ):
    ...
    def create(self, **kwargs) -> Any:
        ...
        if stream:
            ...
            if synth is not None:
                logger.debug("[cacheback] STREAM SYNTHESIS for: %s", query[:80])
        ...
        synth = self._try_synthesis(query, model)
        if synth is not None:
            logger.debug("[cacheback] SYNTHESIS for: %s", query[:80])
            return synth
...
class _AsyncCachedChatCompletions:
    ...
    async def create(self, **kwargs) -> Any:
        ...
        if stream:
            ...
            if synth is not None:
                logger.debug("[cacheback] ASYNC STREAM SYNTHESIS for: %s", query[:80])
        ...
        if synth is not None:
            logger.debug("[cacheback] ASYNC SYNTHESIS for: %s", query[:80])
            return synth
...
class CachedOpenAI:
    ...
    Args:
        cache_dir: Directory for cache data (default: ~/.cacheback/)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            raise ImportError(
                "openai package required. Install with: pip install cacheback-ai[openai]"
            )
