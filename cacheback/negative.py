...
class NegativeCacheAPI:
    ...
    def _lazy_init(self):
        ...
        for row in cursor:
            ...
            try:
                ...
                logger.warning(
                    "[cacheback] Negative cache: failed to embed entry %d: %s",
                    entry_id, exc,
                )
        if rebuilt > 0:
            logger.info("[cacheback] Negative cache: rebuilt %d entries", rebuilt)
    ...
    def add(
        ...
        logger.debug("[cacheback] Negative cache: added entry %d: %s", entry_id, query[:80])
        return entry_id
