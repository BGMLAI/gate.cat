...
| **Google Fonts** | Web font delivery on **one page**, `coverage.html`, which loads a stylesheet from `fonts.googleapis.com` | Visitor IP address and User-Agent | US / global | The main landing page self-hosts its font files but still emits a `preconnect` to `fonts.googleapis.com` and `fonts.gstatic.com`, which opens a connection and therefore discloses the visitor IP. `[TO FIX: drop the preconnect hints and self-host the fonts on coverage.html — then this row disappears entirely]` |
