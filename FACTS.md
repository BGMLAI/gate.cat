...
|---|---|---|---|---|---|---|
| F1 | Recall on dangerous-command corpus | 100% at the 20k checkpoint of the current-gate run; full-1M finish NOT yet read | `scripts/corpus_million.py` + `scripts/corpus_recall.py` (37-class independent catalog). Run #1 (full 1M, pre-fix gate): 0 real misses after reconciliation (5 git-reset = since-fixed real miss, 5 = catalog false alarm). Run #2 (fixed gate+catalog): 100% at 20k, final checkpoint UNREAD (`/opt/gatecat-test/million_checkpoint.json` on the VPS). Measures the `check_action` path (deny walls + delete analyzer) — say so. | TODO: read run #2's final checkpoint and pin it here BEFORE this number appears in any post | until pinned: "internal 1M-command benchmark; eval scripts are in the repo; results artifact publishing next" (C09 fallback). After pinning: "100% recall on a 1M-command corpus for the dangerous set (deny-list + delete-analyzer path)" | "catches everything", "100% safe", quoting 1M/100% before the final checkpoint is read |
