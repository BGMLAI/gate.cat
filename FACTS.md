...
| F3 | Test suite | 1863 passed / 27 skipped / 0 failed locally on Python 3.11; multi-version CI pending for 0.4.16 | local release-gate run: `.venv/bin/python -m pytest -q` (209.44 s); `.github/workflows/ci.yml` will pin py3.11+3.12+3.13 after push | release candidate 0.4.16, 2026-07-13 | "1863 tests pass locally for the 0.4.16 release candidate" until CI completes; then cite the CI run | claiming multi-version CI is green before the run completes |
...
| F9 | Installable version | 0.4.15 | https://pypi.org/project/gate.cat/; 0.4.16 is a release candidate only until publication and clean install from PyPI | 2026-07-13 | "0.4.15 is currently installable from PyPI; 0.4.16 is the tested release candidate" | citing 0.4.16 as installable before PyPI publication |
