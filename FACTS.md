...
| F4 | Bypass suite | 70/70 caught + named gaps printed | `gatecat/integrations/bypass_suite.py` (measure: `python -m gatecat.integrations.bypass_suite`) | 2026-07-09 (was 65/65 at v0.4.0; +5 block dangers from the coverage-audit promotion — IAM esc / backup destroy / HTTP-API DNS-delete) | "bypass suite catches 70/70 of what it claims and prints the gaps it doesn't (base64, deletes via a language runtime, curl\|sh obfuscation)" | "no known bypasses"; the stale 65/65 in NEW copy |
...
| F9 | Installable version |   0.4.9 | https://pypi.org/project/gate.cat/ · tag `v0.4.9` = `572618d`; clean-venv-verified against the PyPI wheel (`__version__` 0.4.9, 28 defaults; RM_RF filename-FP fixed: `rm /tmp/pypirc-fresh` allowed, `rm -rf /srv` blocked; block-outranks-warn: pack block wins over the core generic DELETE warn); Beta + Topic::Security classifiers live | 2026-07-09 | "0.4.9 on PyPI; tag v0.4.9 is the installable version" | citing GitHub-only versions as installable |
| F10 | Default policy walls | 28 in `DOGFOOD_DEFAULTS` (30 presets incl. opt-in) | `gatecat/integrations/policies.py`; measure: `python -c "from gatecat.integrations import DOGFOOD_DEFAULTS; print(len(DOGFOOD_DEFAULTS))"` | 2026-07-09 (was 21 at v0.4.1; +2 KMS/Vault, then +5 coverage-audit promotions: IAM priv-esc + identity-tamper, backup destroy, HTTP-API identity/DNS destroy + generic-delete) | "28 default policies" | the stale "20"/"21"/"23" counts in NEW copy (each is a prior pin: 20 pre-0.4.0, 21 at v0.4.1, 23 after KMS/Vault) |
