...
| F1b | Recall on real agent traffic at scale (FULL gate) | run in progress; interim 4/4 neutralized, 0 passed at 60k unique real commands | `scripts/corpus_million.py` vs `ActionPipeline`, streaming Nemotron/SWE-Zero/SWE-smith/nebius, independent 43-class catalog. Prior run #1 (full 1M, pre-fix gate) reconciled to 0 real misses. | interim master 2026-07-08; final ~1M checkpoint PENDING | until final checkpoint pinned: "internal benchmark hunting for unknown danger shapes in ~1M real commands; scripts in the repo, final number publishing next" (C09 fallback) | a naked "1M / 100%" before the final checkpoint is read and pinned here |
