...
class VectorIndex:
    ...
    def _ensure_index(self):
        ...
        try:
            ...
            logger.warning("[gatecat] hnswlib not available, using brute-force fallback")
        ...
        if self._index_path and os.path.exists(self._index_path):
            try:
                ...
                logger.info("[gatecat] Loaded %d vectors from %s", self._count, self._index_path)
                return
            except Exception as exc:
                logger.warning(
                    "[gatecat] Corrupt index %s: %s — rebuilding fresh",
