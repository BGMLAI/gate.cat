...
class CacheStore:
    ...
    def _init_db(self):
        ...
        try:
            ...
            logger.warning(
                "[gatecat] Corrupt database %s: %s — recreating", self._db_path, exc
            )
            if self._conn:
                try:
                    self._conn.close()
    ...
    def _get_schema_version(self) -> int:
        ...
        try:
            row = self._conn.execute(
                "SELECT value FROM gatecat_meta WHERE key = 'schema_version'"
            ).fetchone()
    ...
    def _set_schema_version(self, version: int) -> None:
        self._conn.execute(
            "INSERT OR REPLACE INTO gatecat_meta (key, value) VALUES ('schema_version', ?)",
            (str(version),),
        )
        self._conn.commit()
    ...
    def _run_migrations(self) -> None:
        ...
        for from_ver, to_ver, sql in MIGRATIONS:
            if current < to_ver:
                logger.info("[gatecat] Migrating schema v%d → v%d", from_ver, to_ver)
