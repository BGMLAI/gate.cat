...
from gatecat.embedders import BaseEmbedder, get_embedder
from gatecat.index import VectorIndex
from gatecat.store import CacheStore
...
class SemanticCache:
    ...
    def _lazy_init(self):
        ...
        if entries and self._index.count == 0:
            logger.info("[gatecat] Rebuilding index from %d stored entries...", len(entries))
            for cache_id, vec in entries:
                self._index.add(vec, cache_id)
            logger.info("[gatecat] Rebuilt index: %d vectors", len(entries))
    ...
    def _embed(self, input_data: Any) -> np.ndarray:
        ...
        try:
            ...
            logger.error("[gatecat] Embedder failed: %s", e)
            raise
    ...
    def lookup(self, query: Any) -> Optional[str]:
        ...
        Raises:
            GatecatBlocked: If query matches negative cache and on_negative_hit="raise".
        """
        ...
        try:
            ...
            if self._negative is not None or self._on_negative_hit != "skip":
                ...
                if neg_hit is not None:
                    from gatecat.exceptions import GatecatBlocked
                    if self._on_negative_hit == "raise":
                        raise GatecatBlocked(
                            query=str(query)[:200],
            ...
            logger.debug(
                "[gatecat] HIT (sim=%.4f, %.1fms, hits=%d) %s",
            ...
            from gatecat.exceptions import GatecatBlocked
            if isinstance(e, GatecatBlocked):
                raise
            # Graceful degradation: cache failure → passthrough
            logger.warning("[gatecat] Lookup failed, passing through: %s", e)
            self._misses += 1
            return None
    ...
    def lookup_for_synthesis(
        ...
        try:
            ...
            logger.warning("[gatecat] Synthesis lookup failed: %s", e)
            return []
    ...
    def populate(self, query: Any, response: str, model: str = "", tokens: int = 0) -> bool:
        ...
        try:
            ...
            logger.warning("[gatecat] Populate failed: %s", e)
            return False
